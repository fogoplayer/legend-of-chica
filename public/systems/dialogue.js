...
class Dialogue extends Phaser.Scene {
     ...
     * Since the dialogue system now uses async/await,I wanted an async function that invoked the Dialog
     * constructor and returned a promise, just to simplify implementation.
        ...
        if (this.dialogIndex < this.dialogList.length) {
            ...
            this.interval = setInterval(() => {
                if (i < dialogText.text.length) {
                    ...
                    if (dialogText.text.substring(i - 1, i) === " ") {
                        lastSpaceIndex = i - 1
                    }

                    //Insert line breaks dynamically
                    if (this.text.width >= 512 * 1.5 - 20) {
                        dialogText.text = dialogText.text.substring(0, lastSpaceIndex) + "\n" + dialogText.text.substring(lastSpaceIndex + 1);
                    }
                }
                else {
                    clearInterval(this.interval);
            ...
            try {
                ...
                console.warn("No resolve function")
            };
        }
