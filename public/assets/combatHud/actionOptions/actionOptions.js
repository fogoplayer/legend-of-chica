...
const actionOptions = {
    ...
    create(_this) {
        ...
        graphics.lineBetween(gameWidth - cellWidth, _this.sys.game.config.height * 2 / 3, gameWidth - cellWidth, gameHeight /*_this.sys.game.config.height / 2 + cellHeight * nOfOptions*/ );

        for (let i = 0; i < nOfOptions; i++) {
            const y = _this.sys.game.config.height * 2 / 3 + cellHeight * (i + .5);
            const x = gameWidth - cellWidth;

            //graphics.lineBetween(x, y - cellHeight / 2, x + cellWidth, y - cellHeight / 2);

            let addedText = _this.add.text(x, y, ` ${ i + 1 } ${ actions[i].name }`, {
                fontSize: '32px',
                fill: '#ffffff',
                color: '#ffffff',
                align: 'left'
